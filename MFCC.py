...
class sound_wave():
    # Constructor: Load the wave file and initialize attributes
    ...
    # Apply the Mel filter bank on the Fourier transformed signal
    ...
    # Calculate the logarithm of the signal power
    def log_power(self):
    ...
    # Apply Discrete Cosine Transform to the log powers
    def DCT(self):
        self.mfcc = dct(self.filter_frames, type=2, axis=1, norm='ortho')[:, 1 : 13]
        (frames_num, coeff_num) = self.mfcc.shape
        lift = 1 + 11 * np.sin(np.pi * np.arange(coeff_num) / 22)
        self.mfcc *= lift
        self.mfcc -= (np.mean(self.mfcc, axis=0) + 1e-8)

    # Stack features horizontally for the final representation
